...
for c, url in companies.items():
    ...
    for disc in discs.find_all('li'):
        try:
            ...
            high_stability = disc.find('span', class_='disc-trait hss has-tip tip-right radius').get_text(strip=True)
            low_stability = disc.find('span', class_='disc-trait lss has-tip tip-right radius').get_text(strip=True)
            company_name = c
            new_d = Disc(name=name, plastic=plastic, difficulty=float(difficulty), speed=float(speed), 
                        glide=float(glide), high_stability=float(high_stability), low_stability=float(low_stability), 
